...
public class LoggingSendObserver : ISendObserver
{
        ...
        where T : class
    {
        ...
        Log.Information(
            "Sending {Message} command from {Namespace}, CorrelationId: {CorrelationId}",
        ...
        where T : class
    {
        ...
        Log.Debug(
            "{MessageType} command from {Namespace} was sent, CorrelationId: {CorrelationId}",
            typeof(T).Name, typeof(T).Namespace, context.CorrelationId);
    }
        ...
        where T : class
    {
        ...
        Log.Error(
            "Fault on sending command {Message} from {Namespace}, Error: {Error}, CorrelationId: {CorrelationId}",
