...
namespace DistributedSystem.Infrastructure.PipelineObservers;
...
public class LoggingReceiveObserver : IReceiveObserver
{
    ...
    public async Task PreReceive(ReceiveContext context)
    {
        ...
        Log.Debug(
            "Receiving message from exchange {Exchange}, Redelivered: {Redelivered}, CorrelationId: {CorrelationId}",
    ...
    public async Task PostReceive(ReceiveContext context)
    {
        ...
        Log.Debug(
            "Message was received from exchange {Exchange}, Redelivered: {Redelivered}, CorrelationId: {CorrelationId}",
            context.TransportHeaders.Get<string>(ExchangeKey), context.Redelivered, context.GetCorrelationId);
    }
    ...
    public async Task PostConsume<T>(ConsumeContext<T> context, TimeSpan duration, string consumerType) where T : class
    {
        ...
        Log.Debug(
            "{Message} message from {Namespace} was consumed by {ConsumerType}, Duration: {Duration}s, CorrelationId: {CorrelationId}",
    ...
    public async Task ReceiveFault(ReceiveContext context, Exception exception)
    {
        ...
        Log.Error(
            "Fault on receiving message from exchange {Exchange}, Redelivered: {Redelivered}, Error: {Error}, CorrelationId: {CorrelationId}",
    ...
    public async Task ConsumeFault<T>(ConsumeContext<T> context, TimeSpan duration, string consumerType, Exception exception) where T : class
    {
        ...
        Log.Error(
            "Fault on consuming message {Message} from {Namespace} by {ConsumerType}, Duration: {Duration}s, Error: {Error}, CorrelationId: {CorrelationId}",
