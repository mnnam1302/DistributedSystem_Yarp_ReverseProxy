...
public class PagedResult<T>
{
    ...
    public static async Task<PagedResult<T>> CreateAsync(IQueryable<T> query, int pageIndex, int pageSize, CancellationToken cancellationToken)
    {
    ...
    public static async Task<PagedResult<T>> CreateMongoQueryableAsync(IMongoQueryable<T> query, int pageIndex, int pageSize, CancellationToken cancellationToken)
    {
        pageIndex = pageIndex <= 0 ? DefaultPageIndex : pageIndex;
        pageSize = pageSize <= 0
            ? DefaultPageSize
        ...
        //var items = await ApplyPagination(query, pageIndex, pageSize).ToListAsync(cancellationToken);
        var items = await query.Skip((pageIndex - 1) * pageSize).Take(pageSize).ToListAsync(cancellationToken);
        return new (items, pageIndex, pageSize, 0);
    }
}
